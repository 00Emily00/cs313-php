...
<?php
    ...
    { //possib
        ...
        echo '<input type="radio" name="' . $x['answers'] . '"  value="' . $row['answers'] . '">' . $row['answers'] . '<br>';
...
$rowsAnswers = $stmt->fetchAll(PDO::FETCH_ASSOC);   
      foreach($rowsAnswers as $rowsAnswer) {
         echo '<input type="radio" name="' . $x['answers'] . '" value="' . $rowsAnswer['answers'] . '">' . $rowsAnswer['answers'] . '<br>';
