...
<?php
    ...
    { //possib
        ...
        if (i % 4 == 0) 
            $animal += '1';
        echo '<div class="radioButton"><label><input type="radio" name="' . $row['questionid'] . '" value="' . $row['answerid'] . '">' . $row['answers'] . '</label></div><br>';
...
$rowsAnswers = $stmt->fetchAll(PDO::FETCH_ASSOC);   
      foreach($rowsAnswers as $rowsAnswer) {
         echo '<div class="radioButton"><label><input type="radio" name="' . $row['questionid'] . '" value="' . $rowsAnswer['answerid'] . '"></label></div>' . $rowsAnswer['answers'] . '<br>';
        }
